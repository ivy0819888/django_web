...
def xuesheng_resetPass(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        try:
            __loginUserColumn__= xuesheng.__loginUserColumn__
        except:
        ...
        eval('''xuesheng.objects.filter({}='{}').update({}='{}')'''.format(username_str,username,password_str,init_pwd))
...
def xuesheng_session(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        msg['data']  = xuesheng.getbyparams(xuesheng, xuesheng, req_dict)[0]

        return JsonResponse(msg)


def xuesheng_default(request):
...
def xuesheng_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if  __isAdmin__ == "是":
            ...
            if tablename!="users" and 'xuesheng'[:7]!='discuss'and "userid" in xuesheng.getallcolumn(xuesheng,xuesheng):
                req_dict["userid"] = request.session.get("params").get("id")
...
def xuesheng_list(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if xuesheng.__tablename__[:7]=="discuss":
            try:
                del req_dict['userid']
            except:
                pass
...
def xuesheng_importExcel(request):
    if request.method in ["POST", "GET"]:
        ...
        if file_type in ['xlsx', 'xls']:
            ...
            try:
                for row in range(1, rows):
                    ...
                    req_dict = {}
                    xuesheng.createbyreq(xuesheng, xuesheng, req_dict)
...
def xuesheng_sendemail(request):
    if request.method in ["POST", "GET"]:
        req_dict = request.session.get("req_dict")
...
def xuesheng_autoSort2(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        leixing = set()
        try:
            cursor.execute("select goodtype from orders where userid = %d"%(request.session.get("params").get("id"))+" and tablename = 'xuesheng' order by addtime desc")
        ...
        L = []
        cursor.execute("select * from xuesheng where $intelRecomColumn in ('%s"%("','").join(leixing)+"') union all select * from xuesheng where $intelRecomColumn not in('%s"%("','").join(leixing)+"')")
...
def xuesheng_value(request, xColumnName, yColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": {}}
        ...
        sql = ''
        if timeStatType == '日':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m-%d') {0}, sum({1}) total FROM xuesheng {2} GROUP BY DATE_FORMAT({0}, '%Y-%m-%d')".format(xColumnName, yColumnName, where, '%Y-%m-%d')

        if timeStatType == '月':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m') {0}, sum({1}) total FROM xuesheng {2} GROUP BY DATE_FORMAT({0}, '%Y-%m')".format(xColumnName, yColumnName, where, '%Y-%m')

        if timeStatType == '年':
            sql = "SELECT DATE_FORMAT({0}, '%Y') {0}, sum({1}) total FROM xuesheng {2} GROUP BY DATE_FORMAT({0}, '%Y')".format(xColumnName, yColumnName, where, '%Y')
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
def xuesheng_o_value(request, xColumnName, yColumnName):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": {}}
        
        where = ' where 1 = 1 '
        
        sql = "SELECT {0}, sum({1}) AS total FROM xuesheng {2} GROUP BY {0}".format(xColumnName, yColumnName, where)
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
def xuesheng_alipay(request):
    if request.method in ["POST", "GET"]:
        ...
        order_string = alipay.api_alipay_trade_page_pay(
            ...
            return_url='http://localhost:8080/djangok981o/xuesheng/notify',
            #notify_url=''
        )
...
def xuesheng_notify(request):
    if request.method in ["POST", "GET"]:
        req_dict = request.session.get("req_dict")
        out_trade_no = req_dict['out_trade_no']
        cursor = connection.cursor()
        
        return redirect('http://localhost:8080/djangok981o/admin/dist/index.html#/xuesheng')
